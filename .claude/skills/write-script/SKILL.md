...
每次编写新脚本时，**首先**用 AskUserQuestion 向用户确认源数据 Excel 的表头结构：

- **标题行数**：Excel 前几行是标题/说明行（非数据、非列名）？常见的有 0 行（第 1 行即列名）、1 行（第 1 行标题，第 2 行列名）
- **英文列名行**：第几行是英文列名（SAS 字段名行）？（1-indexed，如"第 2 行"）

将确认结果记录为 `header` 和 `skiprows`，在后续步骤 3 编写脚本时传给 `load_sheet` / `load_rand`：

```python
# 表头结构：header=0 表示第 1 行为中文列名，skiprows=[1] 跳过第 2 行英文列名
df = load_sheet("DS_END", IMPORT_END, header=0, skiprows=[1])
```

**默认值（无需询问的情况）：** 如果用户说"按已有脚本的格式"或未特别说明，沿用当前项目默认值 `header=0, skiprows=[1]`（即第 1 行中文列名，第 2 行英文列名被跳过）。
