...
- FormField 统一键 `formOID/formName/sasFieldName/itemName/fieldFormat`（+`codeList`）；另有下游必需的附加键：clinflash `fieldOID`、taimei5 `checkedValue`。
...
有则标记 `"hasOther": true`。实现于 `utils/_compat.py` 的 `has_other()`（解析器经
build-metadata.py 顶部 sys.path 引导 import），四个解析器统一调用。
